...
async function processNextMEI() {
	...
	try {
		...
		Object.keys(subscribedStaves).forEach(async (staves) => {
			...
			console.log(
				`[${new Date().toISOString()}] ${COLOR_BLUE}PUBLISHED Staves "${staves}"${COLOR_RESET}`,
			);
		});
	} catch (error) {
...
export function handle_open(ws: any) {
	switch (ws.data.query.type) {
		...
		case "client": {
			...
			const staves = ws.data.query.staves || "all"; // Default to "all" if no staves are specified
			console.log(
				`[${new Date().toISOString()}] ${COLOR_YELLOW}SUBSCRIBED Staves "${staves}"${COLOR_RESET}`,
			);
...
export function handle_message(ws: any, message: any) {
	if (ws.data.query.type === "provider") {
		console.log(
			`[${new Date().toISOString()}] ${COLOR_BLUE}PROVIDER New MEI data${COLOR_RESET}`,
		);
...
export function handle_close(ws: any) {
	switch (ws.data.query.type) {
		...
		case "client": {
			...
			console.log(
				`[${new Date().toISOString()}] ${COLOR_YELLOW}UNSUBSCRIBED Staves "${staves}"${COLOR_RESET}`,
			);
			subscribedStaves[staves] = Math.max(
				(subscribedStaves[staves] || 1) - 1,
				0,
