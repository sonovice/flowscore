  ...
  return (
    <div class="ease-in-out duration-300 relative z-10" role="dialog">
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="relative z-0 flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <button type="button" onClick={props.onClose} onKeyDown={handleBackdropKeyDown} aria-label="Close settings" class="absolute inset-0"></button>
          <div class="z-10 relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            ...
            <div class="bg-white px-4 pb-4 pt-3 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-0">
                  <form>
                    <span class="block text-sm font-medium leading-6 text-gray-900">Select staves</span>
                    ...
                    <div class="mt-4 flex flew-row">
                      <label for="score-size" class="block text-sm font-medium leading-6 text-gray-900">Score size</label>
                    </div>
                    <p class="text-sm text-gray-500">
                      Selects the size of the displayed score.
                    </p>
                    <div class="flex flew-row mt-1 items-center w-full">
                      <input id="score-size" type="range" min="30" max="100" step="1" value={scaleValue()} onInput={(e) => setScaleValue(parseInt(e.target.value, 10))} onChange={handleScoreScaleChange} class="w-full accent-blue-500" />
                    ...
                    <div class="mt-4 flex flew-row">
                      ...
                      <span class="ml-4 block text-sm font-medium leading-6 text-gray-900">Show separator between staves</span>
                    </div>
                    <p class="text-sm text-gray-500">
                    ...
                    <div class="mt-4 flex flew-row">
                      ...
                      <span class="ml-4 block text-sm font-medium leading-6 text-gray-900">Colorize bottom system</span>
                    </div>
                    <p class="text-sm text-gray-500">
                    ...
                    <div class="mt-4 flex flew-row">
                      ...
                      <span class="ml-4 block text-sm font-medium leading-6 text-gray-900">Scroll smoothly</span>
                    </div>
                    <p class="text-sm text-gray-500">
                    ...
                    <div class="mt-4 flex flew-row">
                      ...
                      <span class="ml-4 block text-sm font-medium leading-6 text-gray-900">Smart scrolling</span>
                    </div>
                    <p class="text-sm text-gray-500">
                    ...
                    <div class="mt-4 flex flew-row">
                      <label for="scroll-size" class={`block text-sm font-medium leading-6 ${smartScroll() ? "text-gray-400" : "text-gray-900"}`}>Scroll size</label>
                    </div>
                    ...
                    <div class="flex flew-row mt-1 items-center w-full">
                      <input id="scroll-size" type="range" min="5" max="100" step="1" disabled={smartScroll()} value={scrollValue()} onInput={(e) => setScrollValue(parseInt(e.target.value, 10))} onChange={handleScrollSizeChange} class="w-full accent-blue-500" />
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex items-center justify-between">
              <div>
                <button
                  type="button"
                  disabled={props.isExporting}
                  aria-busy={props.isExporting}
                  class={`inline-flex items-center w-full justify-center ${props.isExporting ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white text-gray-900 hover:bg-gray-50'} ring-gray-300 ring-1 ring-inset rounded-md px-3 py-2 text-sm font-semibold shadow-sm sm:w-auto`}
                  onClick={props.onExport}
                >
                  {props.isExporting && (
                    <svg class="animate-spin -ml-0.5 mr-2 h-4 w-4 text-gray-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                    </svg>
                  )}
                  {props.isExporting ? 'Exporting…' : 'Export PDF'}
                </button>
              </div>
              <div class="flex gap-2 mt-3 sm:mt-0">
                <button type="button" class={`inline-flex justify-center bg-white text-gray-900 ring-gray-300 ring-1 ring-inset rounded-md px-3 py-2 text-sm font-semibold shadow-sm sm:w-auto`} onClick={props.onClear}>Clear</button>
                <button type="button" class={`${props.isConnected ? "bg-blue-500 hover:bg-blue-600" : "bg-red-500 hover:bg-red-600"} inline-flex justify-center rounded-md px-3 py-2 text-sm font-semibold text-white shadow-sm sm:w-auto`} onClick={props.onClose}>Okay</button>
              </div>
            </div>
          </div>
        </div>
