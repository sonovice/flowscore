...
export class VerovioWorkerManager {
	...
	constructor() {
		this.worker = new Worker(new URL("./verovio-worker.ts", import.meta.url), {
			type: "module",
		});
 ...
 */
export function createVerovio(): AsyncToolkit {
	...
	const proxy = new Proxy(manager, {
		get(target, prop) {
			...
			throw new Error(
				`Property ${String(prop)} is not a valid Verovio method.`,
			);
		},
	});
